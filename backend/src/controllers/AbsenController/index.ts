...
class AbsenController implements IController {
  index = async (req: Request | any, res: Response): Promise<Response> => {
    try {
      let tombol_type: string; // masuk, pulang, belum_dimulai, libur(tidak masuk db hanya untuk cek absensi di hari libur)
      let tombol_status: boolean = false;
      let message: string;
      let absen_status: string; // tepat_waktu_masuk, telat, tepat_waktu_pulang, belum_dimulai, libur(tidak masuk db hanya untuk cek absensi di hari libur)
      let sudah_absen: boolean;
      let user_id: number = req.user.id; // diky satria ramadanu (id user yang sedang login)

      // cek absen sekarang libur atau tidak
      let tgl_now = moment();
      let tgl_now_string: string = tgl_now.format("YYYY-MM-DD");
      let cek_libur = await sequelize.query(
        `SELECT * FROM hari_liburs WHERE tanggal = '${tgl_now_string}'`,
        { type: QueryTypes.SELECT }
      );
      if (tgl_now_string === at().tgl_sekarang && cek_libur.length > 0) {
        message = `Hari ini libur ${cek_libur[0].nama}`;
        tombol_type = "libur";
        tombol_status = false;
        absen_status = "libur";
        sudah_absen = false;
      ...
      if (
        ...
        tombol_type = "masuk";
        message = "Tombol masuk muncul dan tepat waktu";
        absen_status = "tepat_waktu_masuk";
        ...
        absen_status = "tepat_waktu_pulang";
